test_that("test inputs", {
  ...
  expect_error(
    sims_simulate("x <- y", nlist(x = NA_real_)),
    "^`constants` must not have any missing values[.]$",
    class = "chk_error"
  )
  expect_error(
    sims_simulate("x <- y", parameters = list(x = TRUE)),
    "^All elements of `parameters` must be numeric[.]$",
    class = "chk_error"
  )
  expect_error(
    sims_simulate("x <- y", parameters = list(x = NA_real_)),
    "^`parameters` must not have any missing values[.]$",
    class = "chk_error"
  )
  expect_error(
    sims_simulate("x <- y", list(x = 1), monitor = 1),
...
test_that("latent, stochastic nodes", {
  ...
  expect_equal(
    ...
    nlist::nlists(nlist(
      x = 0.574752916346771,
      x2 = 0.912021637658974,
      y = 2,
      y2 = 0.912021637658974
    ))
  )
  withr::local_seed(101)
  expect_equal(
...
test_that("with R code", {
  ...
  result <- sims_simulate("a <- TRUE", stochastic = NA)
...
test_that("with R code and single stochastic node", {
  ...
  expect_error(
    sims_simulate("a <- runif(1, 0, 1)", stochastic = FALSE, latent = FALSE),
